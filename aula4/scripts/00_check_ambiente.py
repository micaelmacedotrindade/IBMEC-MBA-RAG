"""
00_check_ambiente.py - Verifica se o ambiente da Aula 4 esta pronto.

Testa (um item por linha, com OK ou FALHA):
  1. Python
  2. Bibliotecas (haystack, hybrid retriever, joiner, transformers p/ SPLADE, langfuse, etc.)
  3. .env e chaves
  4. Corpus + queries da Aula 4
  5. OpenSearch + plugin kNN
...
def checar_bibliotecas():
    ...
    libs = [
        ...
        "transformers",                                # SPLADE (neural sparse)
...
def checar_corpus():
    print("4) Corpus + queries da Aula 4")
    try:
        docs = _comum.carregar_corpus()
        queries = _comum.carregar_queries()
        ok(f"{len(docs)} documentos | {len(queries)} queries de avaliacao")
    except Exception as e:
        falha(f"nao consegui ler corpus/queries -> {e}")
...
def checar_langfuse():
    ...
    if not _comum.langfuse_configurado():
        aviso("chaves do LangFuse ausentes - tracing desligado (veja GUIA_LANGFUSE_WINDOWS.md da Aula 3)")
        return
...
def main():
    parser = argparse.ArgumentParser(description="Verifica o ambiente da Aula 4.")
    parser.add_argument("--testar-groq", action="store_true")
    args = parser.parse_args()

    print("=" * 60)
    print("  CHECAGEM DE AMBIENTE - MBA RAG & CAG - Aula 4")
    print("=" * 60)
    checar_python()
    checar_bibliotecas()
    checar_env()
